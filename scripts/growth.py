...
def country(name, lastndays=12, plot=False):
    ...
    cumulative = frame[frame['Country/Region'] == name].values.squeeze()[1:][-lastndays:].astype(numpy.float64)
...
def median_age():
    df = pandas.read_html('https://en.wikipedia.org/wiki/List_of_countries_by_median_age')
    return df[0]
